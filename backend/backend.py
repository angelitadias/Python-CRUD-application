...
class TransactionObject() :
    ...
    def execute (self, sgl, parms = None) :
        if TransactionObject.connected:
            ...
            if parms == None:
                ...
                TransactionObject.cur.execute(sgl, parms)
                return True
...
def insert (nome, sobrenome, email, epf) :
    trans = TransactionObject()
    trans.connect()
...
def update (id, nome, sobrenome, email, cpf) :
    ...
    trans.execute ("UPDATE clientes SET nome =?, sobrenone=?, email=?. cpf=? WHERE id = ?", (nome, sobrenome, email, cpf, id)) 
    trans.persist ()
    trans.disconnect()
